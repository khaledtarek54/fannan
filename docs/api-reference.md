...
| Method | URI | Controller | Auth | Notes |
|--------|-----|------------|------|-------|
| POST | `/payment/checkout` | `PaymentController@checkout` | `auth:api` + CompleteProfile | HyperPay checkout |
| POST | `/payment/status` | `PaymentController@checkPaymentStatus` | `auth:api` + CompleteProfile | trusts `resourcePath` (see security) |
| POST | `/payment-webhook` | `PaymentController@webhook` | **inside** auth group | HyperPay callback |
| POST | `/checkout` | `PaymentController@checkout` | **public (outside auth)** | ⚠️ duplicate, unauthenticated |
| GET | `/webhook` | `PaymentController@webhook` | **public** | ⚠️ |
| POST | `/easykash/pay` | `EasyKashController@createPayment` | **public** | create EasyKash pay link |
| GET/POST | `/easykash/callback` | `EasyKashController@callback` | **public** | POST verifies HMAC; GET redirect does not |
| GET | `/payments/easykash/status` | closure (`routes/api.php:30`) | **public** | reference lookup (enumeration) |
...
| Invoices | `apiResource('invoices')` — **public**, methods mostly unimplemented |
| Command | `GET /command/{command}` — ⚠️ **runs `Artisan::call()`, public** |

## Web routes (`routes/web.php`)

`/` → `/admin`; legal pages (`/privacy-policy`, `/terms`, `/about`, `/contact` + `POST /contact-store`); `/delete-account` view + `POST /delete` (delete by phone); `/artist-register` + `POST /artist-store`; `/payments/easykash/return`; debug routes `/test-mail`, `/firebase-test`.
...
> **Important:** role checks confirm *what kind* of user is calling, not *whether they own the target record*. The IDOR findings in [SECURITY_ISSUES.md](SECURITY_ISSUES.md) stem from missing **ownership** checks in controllers/services even when a role check passes.
