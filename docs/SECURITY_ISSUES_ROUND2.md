...
| R2-M1 | `customer_reference` is `rand(100000,999999)` — guessable + collision-prone, no unique constraint | Medium | ⚠️ | ☑ |
...
**Fixed (`security/round2-criticals`):** ☑ New `FirebaseAuthService::verifiedEmail()` verifies the Firebase ID token via the kreait Admin SDK (`verifyIdToken`, `checkIfRevoked=true`, requires `email_verified`) and returns the token's verified email. `SocialLoginRequest` now requires `id_token` (no trusted `email`); `UserRepository::socialLogin()` resolves the account from the verified email, stays **login-only** (rejects unknown emails, no auto-register). Covered by `tests/Feature/SocialLoginTest.php`. **Feature switch:** the endpoint is behind `config('auth.social_login_enabled')` (env `SOCIAL_LOGIN_ENABLED`, **default false**). While off, `SocialLoginRequest::authorize()` short-circuits before validation and returns a clean 503 `auth.social_login_disabled` — never the old email-trust path. Flip `SOCIAL_LOGIN_ENABLED=true` in the env (no code deploy) once the mobile app ships a build that sends the Firebase ID token as `id_token`. **Ops note:** enabling also requires `FIREBASE_CREDENTIALS` (service-account JSON) in prod (already present: project `fannan-ac558`).
...
**Fixed (`security/round2-criticals`):** ☑ New `CounterpartyResource` is the "other party" shape — it never emits `fcm_token`, `vat_number`, `cr_number`, DOB or gender, and emits `email`/`phone`/`phone_prefix`/`whatsapp` only when constructed with `$withContact = true`. All four cross-user embeds now use it: bidding browse (`BiddingOrderResource.client`) → never contact; bidding offers (`BiddingOrderOfferResource.artist`) → contact only when the offer `is_accepted`; direct/accepted orders (`OrderResource.artist`, `ArtistOrderResource.client`) → contact only when the order is a confirmed engagement (`is_paid` or ACCEPTED/IN_PAYMENT/COMPLETED). **Owner decision:** counterparty contact is shown **only for confirmed engagements**. The `ClientResource` `vat/cr = $this->name` copy-paste bug is fixed (it is now the client's self-profile only). Covered by `tests/Feature/PiiRedactionTest.php`.
...
**Fixed (`security/round2-criticals`):** ☑ `config/telescope.php` default flipped to `env('TELESCOPE_ENABLED', false)` — an env that merely omits the flag no longer records anything. `TelescopeServiceProvider::hideSensitiveRequestDetails()` now scrubs `password`, `password_confirmation`, `verification_code`, `fcm_token`, `token`, `id_token`, `access_token` (was only `_token`) and hides the `authorization` header. *Note: prod already had `TELESCOPE_ENABLED=false` + cleared tables (2026-07-02); this makes the safe state the default. The dashboard gate still hardcodes admin emails — tightening that is a separate item.*
...
## Group 5 — fixes applied (`security/round2-fixes`)

Covered by `tests/Feature/HardeningTest.php` unless noted.
- **R2-H5** ☑ — `ClientCompleteProfileRequest` now validates `profile_photo` as `file|mimetypes:image/jpeg,image/png,image/webp|max:5120`; the filename was already server-generated by `Storage::put()`. (test: non-image → 422, image accepted.)
- **R2-M1** ⚠️☑ — generation switched from `rand(100000,999999)` to `random_int(10^11,10^12)`. **Doc correction:** the `customer_reference` column *already* has a `UNIQUE` index (`2025_12_10` migration), so the collision→wrong-settlement risk was already closed; this only removes the weak/guessable PRNG. (A collision now fails the insert rather than mis-settling.)
- **R2-M3** ☑ — `OrderPricingService::breakdown` clamps `discount` to `[0, cost+tax]` and floors `total_cost` at 0; `calculateCouponAmount` clamps the discount to the cost. *Not done:* checkout-time coupon reservation / global redemption cap (a larger change — left as a follow-up; per-user reuse is already blocked by `couponUserRepository->checkIfExists`).
- **R2-M4** ☑ — `getTransactionStatus()` now queries `customer_reference` (was `order_id`).
- **R2-M5** ☑ (accepted) — `/api/check-phone-exists` **kept** (owner decision): the mobile signup flow relies on it, it is now under `throttle:auth` keyed by `phone|ip` (R2-M2), and registration already reveals a taken phone via unique validation — so removing it would not close the vector.
- **R2-L1** ☑ — `git rm` of `routes/api.php.bak-*` + `RouteServiceProvider.php.bak-*` (old RCE route).
- **R2-L2** ☑ — deleted unrouted `OrderOfferController` + `OrderOfferService`; removed the dead `POST /support/delete` route (`SupportController::destroy` didn't exist). (test: `/support/delete` → 404.)
- **R2-L3** ☑ — `git rm public/default.php` + `public/default.php.old.php` (the `payment-success/failed.html` pages are functional and kept).
- **R2-L4** ☑ (accepted, low) — CORS left as-is: it applies to `api/*` (bearer-token) with `supports_credentials=false`, and native mobile clients send no `Origin`, so browsers won't attach credentials. Tightening origins remains optional defense-in-depth.
- **R2-L5** ☑ — added `$hidden` to `UserTransaction` (`callback_payload`, `easykash_ref`, `email`, `mobile`) and `Transaction`; `SupportController::store` now returns a `SupportResource` instead of the raw model.
